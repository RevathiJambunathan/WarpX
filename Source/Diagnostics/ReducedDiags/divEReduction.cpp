...
// constructor
divEReduction::divEReduction (std::string rd_name)
: ReducedDiags{rd_name}
{
    ...
    utils::parser::Store_parserString(pp_rd_name,"reduced_function(x,y,z)",
                       parser_string);
    m_parser = std::make_unique<amrex::Parser>(
        utils::parser::makeParser(parser_string,{"x","y","z"}));
...
void divEReduction::ComputeDiags (int step)
{
    ...
    {
        ComputedivEReduction<amrex::ReduceOpMax>();
    }
    else if (m_reduction_type == ReductionType::Minimum)
    {
        ComputedivEReduction<amrex::ReduceOpMin>();
    }
    else if (m_reduction_type == ReductionType::Sum)
    {
        ComputedivEReduction<amrex::ReduceOpSum>();
    }
